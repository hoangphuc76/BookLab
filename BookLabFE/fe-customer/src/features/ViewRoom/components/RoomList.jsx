              ...
              >
                ...
                >
                  ...
                  {room.imageUrls && room.imageUrls.length > 1 && (
                    <>
                      <button
                        className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center rounded-full bg-white/20 backdrop-blur-sm hover:bg-white/40 transition-colors border border-white/30 shadow-lg opacity-0 group-hover:opacity-100"
                        onClick={(e) => {
                          e.stopPropagation();
                      ...
                      <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-1.5">
                        {room.imageUrls.map((_, dotIndex) => (
                          <button
                            key={dotIndex}
                            className={`h-2 w-2 rounded-full transition-colors ${(currentImageIndex[room.id] || 0) === dotIndex
                              ? "bg-white"
                              : "bg-white/40"
                              }`}
                            onClick={(e) => {
