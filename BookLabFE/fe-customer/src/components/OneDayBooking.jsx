...
const slotColors = {
  0: "rgba(255, 82, 82, 0.2)",
  1: "rgba(255, 82, 82, 0.5)",
  2: "rgba(0, 200, 83, 0.2)",
  3: "rgba(0, 200, 83, 0.5)",
}
...
const lineColor = {
  ...
  4: "#e91e63",
  5: "rgba(255, 0, 0, 0.8)",
  6: "rgba(0, 123, 255, 0.8)",
}

const groupPointsColor = {
  0: "#c2185b",
  1: "#e91e63",
}
...
const OneDayBooking = ({ room = null, dateDay = null, typeSlot = 3, day = null, handleChangeView, handleSetCurrentEvent, handleOpenSubBookingDetailIOneDay, size }) => {
  ...
  const processedData = useMemo(() => {
    ...
    let originalLecturerCodeData = [];
    let originalPrivateData = [];
    let originalInternalData = [];
    let originalMaintenanceData = [];
    ...
    const addTimeSlot = (
      ...
      if (!timeMap.has(timeKey)) {
        ...
        originalLecturerCodeData.push([...lecturerCode])
        originalPrivateData.push(privateStatus);
        originalInternalData.push(internalStatus);
        originalMaintenanceData.push(maintenanceStatus);
        originalLockData.push(lockStatus);
      }
    };
    ...
    const updateTimeSlot = (
      ...
      if (timeMap.has(timeKey)) {
        ...
        originalLecturerCodeData[index].push(...lecturerCode);
        // originalPrivateData.push(privateStatus);
      }
    };
    ...
    if (dateDay) {
      ...
      sortedBookings.forEach((booking, index) => {
        ...
        let lecturerId = [booking.lectureId];
        let lecturerCode = [booking.lectureCode]
        let privateStatus = booking.private ?? false;
        let internalBooking = booking.type == 6;
        let maintenanceBooking = booking.approve == 7;
        let lockBooking = booking.approve == 8;
        ...
        if (originalTimePeriods.length == 0) {
          ...
          let resultLecturerCodeData = [...originalLecturerCodeData];
          let resultPrivateData = [...originalPrivateData];
          let resultInternalData = [...originalInternalData];
          let resultMaintenanceData = [...originalMaintenanceData];
          let resultLockData = [...originalLockData];
          ...
          if (affectedSlots.length > 0) {
            ...
            resultLecturerCodeData.splice(
              index,
              1,
              originalLecturerCodeData[index],
              [...originalLecturerCodeData[index], ...lecturerCode],
              originalLecturerCodeData[index]
            );
            ...
            if (!timeMap.has(timeKey)) {
              ...
              resultLecturerCodeData.push([...lecturerCode]);
              resultPrivateData.push(privateStatus);
              resultInternalData.push(internalBooking);
              resultMaintenanceData.push(maintenanceBooking);
              resultLockData.push(lockBooking);
            }
          } else {
            for (let i = 0; i < splitTimes.length - 1; i++) {
              ...
              if (originalTimePeriods.includes(key)) {
                ...
                resultLecturerCodeData[index].push(...lecturerCode)
              } else {
                ...
                if (checkStartTime === plusStartTime) {
                  if (plusEndTime < checkEndTime) {
                    ...
                    resultLecturerCodeData.splice(
                      indexPlus,
                      1,
                      [...originalLecturerCodeData[i], ...lecturerCode],
                      originalLecturerCodeData[i]
                    );
                    resultPrivateData.splice(
                      i,
                      1,
          ...
          originalLecturerCodeData = resultLecturerCodeData;
          originalPrivateData = resultPrivateData;
          originalInternalData = resultInternalData;
          originalMaintenanceData = resultMaintenanceData;
          originalLockData = resultLockData;
          timeMap.clear();
        ...
        console.log("Afterrrrrrrrrrrrrrrrrrr: ", originalLecturerCodeData);
        console.log("Afterrrrrrrrrrrrrrrrrrr: ", originalPrivateData);
        console.log("Afterrrrrrrrrrrrrrrrrrr: ", originalInternalData);
        console.log("Afterrrrrrrrrrrrrrrrrrr: ", originalMaintenanceData);
        console.log("Afterrrrrrrrrrrrrrrrrrr: ", originalLockData);
      });
    }
    ...
    return {
      ...
      originalLecturerCodeData,
      originalPrivateData,
      originalInternalData,
      originalMaintenanceData,
      originalLockData,
      timeMap,
    }
  ...
  const { originalTimePeriods, originalStudentsData, originalGroupsData, originalLecturerData, originalLecturerCodeData, originalPrivateData, originalInternalData, originalMaintenanceData, originalLockData } =
    processedData
  ...
  const timeDetails = Array(originalTimePeriods.length)
    ...
    .map((_, i) => {
      ...
      return {
        ...
        maintenance: originalMaintenanceData[i],
        lock: originalLockData[i],
        teacher: originalLecturerCodeData[i],
        isFullyBooked,
        remainStudent: room.capacity - originalStudentsData[i],
        remainGroup: room.groupSize - originalGroupsData[i]
      }
    })
  ...
  // Function to maintenance in range
  const checkMaintenance = (startIndex, endIndex) => {
    for (let i = 0; i < originalTimePeriods.length; i++) {
      if (timeDetails[i].maintenance) {
        const period = getIndexOfFullTime(i)

        // Check if this period overlaps with the selection range
        if (!(period.endIndex < startIndex || period.startIndex > endIndex)) {
          return {
            startMaintenanceIndex: period.startIndex,
            endMaintenanceIndex: period.endIndex,
    ...
    return { statusMaintenance: false }
  }

  // Function to lock in range
  const checkLock = (startIndex, endIndex) => {
    for (let i = 0; i < originalTimePeriods.length; i++) {
      if (timeDetails[i].lock) {
        const period = getIndexOfFullTime(i)

        // Check if this period overlaps with the selection range
        if (!(period.endIndex < startIndex || period.startIndex > endIndex)) {
          return {
            startLockIndex: period.startIndex,
            endLockIndex: period.endIndex,
            statusLock: true
          }
        }
      }
  ...
  // Tìm giao điểm của đường thẳng với các cạnh của hình chữ nhật
  const findIntersections = (x0, y0, dx, dy, rectX, rectY, rectWidth, rectHeight) => {
    // Mảng lưu các giao điểm
    const intersections = []

    // Kiểm tra giao với cạnh trên (y=rectY)
    if (dy !== 0) {
      const t = (rectY - y0) / dy
      const x = x0 + t * dx
    ...
    // Kiểm tra giao với cạnh dưới (y=rectY+rectHeight)
    if (dy !== 0) {
      const t = (rectY + rectHeight - y0) / dy
      const x = x0 + t * dx
      if (x >= rectX && x <= rectX + rectWidth) {
        intersections.push({ x, y: rectY + rectHeight })
      }
    }

    // Kiểm tra giao với cạnh trái (x=rectX)
    if (dx !== 0) {
      const t = (rectX - x0) / dx
      const y = y0 + t * dy
    ...
    // Kiểm tra giao với cạnh phải (x=rectX+rectWidth)
    if (dx !== 0) {
      const t = (rectX + rectWidth - x0) / dx
      const y = y0 + t * dy
      if (y >= rectY && y <= rectY + rectHeight) {
        intersections.push({ x: rectX + rectWidth, y })
      }
    }

    // Trả về 2 giao điểm (nếu có)
  ...
  useEffect(() => {
    const updateCanvasSize = () => {
      if (containerRef.current) {
        ...
        containerRef.current.style.width = `${size.width}%`
  ...
  useEffect(() => {
    ...
    slots?.forEach((slot, slotIndex) => {
      ...
      if (startX > 0 && endX > 0) {
        ...
        const { statusMaintenance } = checkMaintenance(startIndex, endIndex)
        const { statusLock } = checkLock(startIndex, endIndex)
        const isUnavailable = hasFullyBooked || statusPrivate || statusInternal || statusMaintenance || statusLock
    ...
    slotPositions?.forEach((slot) => {
      ctx.fillStyle = slot.isUnavailable ? slotColors[0] : slotColors[2]
      ctx.fillRect(slot.x - 25, padding - 20, 50, 15)

      ctx.strokeStyle = slot.isUnavailable ? slotColors[1] : slotColors[3]
      ctx.lineWidth = 2
    ...
    originalStudentsData.forEach((value, i) => {
      const barHeight = originalInternalData[i] || originalMaintenanceData[i] || originalLockData[i] ? chartHeight : (value / studentsScale.max) * chartHeight
      ...
      if (originalInternalData[i] && (tooltip.visible && tooltip.timePeriod === originalTimePeriods[i])) {
        ctx.fillStyle = typeColors[0] // Dark pink for hover
      } else if ((originalPrivateData[i] || timeDetails[i].isFullyBooked) && (tooltip.visible && tooltip.timePeriod === originalTimePeriods[i])) {
        ctx.fillStyle = typeColors[1] // Darker red for hover
      } else if (tooltip.visible && tooltip.timePeriod === originalTimePeriods[i]) {
        ctx.fillStyle = typeColors[2] // Darker green for hover
      } else if (originalMaintenanceData[i] || originalLockData[i]) {
        ctx.fillStyle = typeColors[3]
        ...
        ctx.fillStyle = typeColors[4] // Light pink for internal
      } else if (originalPrivateData[i] || timeDetails[i].isFullyBooked) {
        ctx.fillStyle = typeColors[5] // Red for fully booked
      } else {
        ctx.fillStyle = typeColors[6] // Light green color
      }

      // Only draw if there's a value
      if (originalMaintenanceData[i] || originalLockData[i]) {
        ctx.fillStyle = typeColors[3]
        ctx.fillRect(x, y, barWidth, barHeight)
        ctx.beginPath()
        ctx.strokeStyle = lineColor[2]
        ...
        // Tính toán vector đơn vị của đường chéo chính
        const diagonalLength = Math.sqrt(barWidth * barWidth + barHeight * barHeight)
        const dx = barWidth / diagonalLength
        const dy = barHeight / diagonalLength

        // Tính toán vector vuông góc với đường chéo chính (để di chuyển song song)
        const perpDx = -dy
        const perpDy = dx

        // Đường chéo chính màu đỏ
        ctx.beginPath()
        ctx.strokeStyle = lineColor[3]
        ctx.moveTo(reactX, reactY)
        ctx.lineTo(reactX + barWidth, reactY + barHeight)
        ctx.stroke()

        // Tính số lượng đường chéo cần vẽ ở mỗi bên của đường chéo chính
        const numLines = Math.ceil(diagonalLength / 10)

        // Vẽ các đường chéo song song với đường chéo chính
        ctx.strokeStyle = lineColor[3]

        // Vẽ các đường phía trên/trái đường chéo chính
        for (let i = 1; i <= numLines; i++) {
          const offset = i * 10

          // Điểm tham chiếu (dịch chuyển từ gốc tọa độ theo vector vuông góc)
          const refX = reactX + offset * perpDx
          const refY = reactY + offset * perpDy

          // Tìm giao điểm của đường thẳng đi qua điểm tham chiếu và song song với đường chéo chính
          const intersections = findIntersections(refX, refY, dx, dy, reactX, reactY, barWidth, barHeight)
        ...
        // Vẽ các đường phía dưới/phải đường chéo chính
        for (let i = 1; i <= numLines; i++) {
          const offset = i * 10

          // Điểm tham chiếu (dịch chuyển từ gốc tọa độ theo vector vuông góc ngược lại)
          const refX = reactX - offset * perpDx
          const refY = reactY - offset * perpDy
    ...
    ctx.strokeStyle = lineColor[4] // Pink color
    ctx.lineWidth = 2
    ctx.stroke()
    ...
    originalGroupsData.forEach((value, i) => {
      ...
      if (value > 0) {
        ...
        if (tooltip.visible && tooltip.timePeriod === originalTimePeriods[i]) {
          ctx.fillStyle = groupPointsColor[0] // Darker pink for hover
        } else {
          ctx.fillStyle = groupPointsColor[1] // Pink color
        }
    ...
    if (isDragging && dragStart !== null && dragEnd !== null) {
      ...
      if (startX >= padding && endX <= width - padding) {
        ...
        ctx.fillStyle = (hasFullyBooked || statusPrivate || statusInternal || statusMaintenance || statusLock) ? bookingColor[0] : bookingColor[1]
        ctx.fillRect(startX, padding - 5, endX - startX, 10)

        // Draw border
        ctx.strokeStyle = (hasFullyBooked || statusPrivate || statusInternal || statusMaintenance || statusLock) ? lineColor[5] : lineColor[6]
        ctx.lineWidth = 2
    ...
    // Fully booked legend
    ctx.fillStyle = typeColors[5] // Red color
    ctx.fillRect(padding + 250, legendY, 20, 10)
    ctx.fillStyle = fillColors[1]
    ctx.fillText("Full || Private", padding + 280, legendY + 5)

    // Internal booked legend
    ctx.fillStyle = typeColors[4] // Pink color
    ctx.fillRect(padding + 410, legendY, 20, 10)
    ctx.fillStyle = fillColors[1]
    ctx.fillText("Internal", padding + 440, legendY + 5)

    // Maintenance || lock booked legend
    ctx.fillStyle = typeColors[3]
    ctx.fillRect(padding + 500, legendY, 20, 10)
    ctx.beginPath()
    ctx.strokeStyle = lineColor[2]
    ctx.lineWidth = 2
    ctx.rect(padding + 500, legendY, 20, 10)
    ctx.stroke()

    ctx.lineWidth = 2
    // Tính toán vector đơn vị của đường chéo chính
    const diagonalLength = Math.sqrt(20 * 20 + 10 * 10)
    const dx = 20 / diagonalLength
    const dy = 10 / diagonalLength
    ...
    ctx.beginPath()
    ctx.strokeStyle = lineColor[3]
    ctx.moveTo(padding + 500, legendY)
    ctx.lineTo(padding + 520, legendY + 10)
    ctx.stroke()

    // Tính số lượng đường chéo cần vẽ ở mỗi bên của đường chéo chính
    const numLines = Math.ceil(diagonalLength / 5)
    ...
    for (let i = 1; i <= numLines; i++) {
      ...
      // Điểm tham chiếu (dịch chuyển từ gốc tọa độ theo vector vuông góc)
      const refX = padding + 500 + offset * perpDx
      const refY = legendY + offset * perpDy

      // Tìm giao điểm của đường thẳng đi qua điểm tham chiếu và song song với đường chéo chính
      const intersections = findIntersections(refX, refY, dx, dy, padding + 500, legendY, 20, 10)
    ...
    ctx.fillStyle = fillColors[1]
    ctx.fillText("Maintenance || Lock", padding + 530, legendY + 5)

    // Draw booking instruction
    ctx.fillStyle = fillColors[3]
    ctx.textAlign = "center"
  ...
  const handleCanvasMouseMove = (event) => {
    ...
    if (hoveredBar) {
      ...
      showTooltip(index, hoveredBar.x + hoveredBar.width * 3 / 4 + 40, hoveredBar.y + hoveredBar.height / 2 + 80)
      canvas.style.cursor = "pointer"
    } else {
  ...
  const simulateSlotSelection = (startIndex, endIndex) => {
    ...
    // Check maintenance 
    const { statusMaintenance, startMaintenanceIndex, endMaintenanceIndex } = checkMaintenance(startIndex, endIndex)

    // Check lock 
    const { statusLock, startLockIndex, endLockIndex } = checkLock(startIndex, endIndex)
    ...
    if (duplicateBooking) {
      ...
      const timeRangeMaintence = getTimeRangeFromIndices(startMaintenanceIndex, endMaintenanceIndex)
      swtoast.warning({
        text: `Unable to book: Time slot ${timeRangeMaintence} is maintenance`,
      })
    } else if (statusLock) {
      const timeRangeLock = getTimeRangeFromIndices(startLockIndex, endLockIndex)
      swtoast.warning({
        text: `Unable to book: Time slot ${timeRangeLock} is lock`,
      })
      ...
      if (startIndex !== -1 && endIndex !== -1 && startIndex < endIndex) {
        ...
        if (duplicateBooking) {
          ...
          const bookDetail = {
            ...
            endTime: timeRange.split("-")[1] + ":00",
            timeRange,
            studentsLeft: availableStudents - 1,
            groupsLeft: availableGroups - 1,
            studentQuantity: 0,
            groupQuantity: 0,
            lectureId: userId,
            typeSlot: typeSlot,
  ...
  return (
    <div
      ref={containerRef}
      className="flex flex-col items-center p-4 w-full relative bg-white/50 backdrop-blur-md 
                rounded-3xl min-w-[500px] h-[750px] border-2 border-gray-300 shadow-2xl"
    >
        ...
        {tooltip.visible && (
          ...
          >
            ...
            <div className="flex justify-between mb-1">
              ...
              <span className="font-bold">{timeDetails[originalTimePeriods.indexOf(tooltip.timePeriod)]?.remainStudent}</span>
            </div>
            {room.onlyGroupStatus && (<div className="flex justify-between mb-1">
              <span>Available Groups:</span>
              <span className="font-bold">{timeDetails[originalTimePeriods.indexOf(tooltip.timePeriod)]?.remainGroup}</span>
            </div>)}
