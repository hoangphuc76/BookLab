...
import {
  HiX, HiPlus, HiChevronDown, HiCheckCircle, HiOutlineRefresh,
...
const CreateGroupModal = ({
  ...
  const handleSaveGroups = async () => {
    ...
    Object.keys(groupsToProcess).forEach(groupKey => {
      if (groupsToProcess[groupKey].length == 0) {
        emptyGroup = groupKey
        return;
      }
    ...
    if (emptyGroup) {
      swtoast.error({
        title: "Invalid Group",
        text: `Group ${emptyGroup} is empty`,
        timer: 3000
      })
      return;
    }
    ...
    try {
      ...
      swtoast.error({ text: `${error.response?.data?.message || 'Something went wrong'}`, timer: 3000 });
    }
  };
              ...
              <div className="p-4 border-t border-slate-100 flex-shrink-0">
                <div className="flex justify-end space-x-2">
                  ...
                  {(isAddNewGroup || isCreateNewGroupInCart || typeModal == 1) && (
                    ...
                    >
                      {load ? (
                        <HiOutlineRefresh className="animate-spin" size={12} /> // Spinning icon
                      ) : (
                        <HiCheckCircle size={12} /> // Normal check circle
                      )}
                      {load ? "Saving..." : "Save"}
                    </button>
                  )}
