...
const ChangeRoomStatusForm = ({ room, isOpen, setIsOpen, onSubmit, onCancel, isResetButtonDisplay }) => {
  ...
  const [isResetButtonWork, setIsResetButtonWork] = useState(false); // State to control reset button
  ...
  const handleResetStatus = async () => {
    try {
      ...
      // Call the correct reset-status endpoint with roomId in the path
      await ApiClient.put(`/Room/${room.id}/reset-status`);

      // Show success message
      swtoast.success({
        title: 'Success',
        text: 'Room status has been reset to its default state'
      });
      ...
      swtoast.error({
        title: 'Error',
        text: error.response?.data?.message || 'Failed to reset room status'
      });
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (room) {
      ...
      if (isResetButtonDisplay) {
        setIsResetButtonWork(room.roomStatus !== 1); // Enable reset button if the room is not locked or in maintenance mode
      } else {
        setIsResetButtonWork(isResetButtonDisplay); // Set the reset button state based on room data
      }
      setMaintenanceMode(false);
      setRoomLocked(false);
      setLoading(false);
  ...
  const handleSubmit = async (e) => {
    ...
    const minimumStartDate = moment().add(3, 'days').startOf('day');

    if (dateRange.startDate.isBefore(minimumStartDate)) {
      swtoast.error({
        title: 'Invalid Start Date',
        text: 'Room status changes must be scheduled at least 3 days in advance'
      });
      return;
    }
        ...
        <form onSubmit={handleSubmit}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <DatePicker
                    ...
                    getPopupContainer={(trigger) => trigger.parentElement} // This helps with positioning
                    className="w-full"
          ...
          <div className="flex justify-between space-x-4">
            ...
            <button
              ...
              className={`inline-flex items-center justify-center py-2.5 px-5 border rounded-xl shadow-sm text-sm font-medium ${!isResetButtonWork
                ? 'border-gray-200 text-gray-400 bg-gray-50 cursor-not-allowed'
                : 'border-red-300 text-red-700 bg-white hover:bg-red-50'
                }`}
              disabled={loading || !isResetButtonWork}
              title={!isResetButtonWork ? 'Reset function not available for this room' : 'Reset room status'}
            >
              <svg
                className={`w-4 h-4 mr-2 ${!isResetButtonWork ? 'text-gray-400' : ''}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
