...
import DefaultImg from '../assets/Default_img.jpg'; // Đường dẫn đến ảnh mặc định
...
const ReusableForm = ({ fields, initialValues, onSubmit, onCancel, isOpen, setIsOpen, campusData, roleData, categoryRoomData, buildingData }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (isAccountForm) {
      ...
      // Auto-fill avatar if null
      const avatarField = fields.find(f =>
        f.name.toLowerCase().includes('avatar') ||
        f.name.toLowerCase().includes('image') ||
        f.name.toLowerCase().includes('photo')
      );
      
      if (avatarField && (!finalFormData[avatarField.name] || finalFormData[avatarField.name] === null)) {
        // Use a default avatar URL directly
        const defaultAvatarUrl = DefaultImg; // Path to your default image
        finalFormData[avatarField.name] = defaultAvatarUrl;
        
        // Set up the default image file for submission
        // This should be done asynchronously before form submission
        fetch(defaultAvatarUrl)
          .then(response => response.blob())
          .then(blob => {
            const defaultAvatarFile = new File([blob], 'default-avatar.jpg', { type: 'image/jpeg' });
            setImageFile(defaultAvatarFile);
          })
          .catch(error => {
            console.error('Error loading default avatar file:', error);
          });
      }
    }
