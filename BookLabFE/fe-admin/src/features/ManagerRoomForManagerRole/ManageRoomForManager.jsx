...
import ReusableTable from '../../components/ReuseTable';
...
const RoomManager = () => {
    ...
    const handleEdit = (item) => {
        // Check if we have an editMode from dropdown selection
        if (item.editMode === 'status') {
            setEditingItem(item);
    ...
    const handleStatusSubmit = async (statusData) => {
        try {
          ...
          await apiClient.put('/Room/status/temporary', null, {
            params: {
              roomId: statusData.roomId,
              status: statusData.statusValue,
              startDate: statusData.startDate,
              endDate: statusData.endDate
            }
          });
      
          // Show success message using swtoast
          swtoast.success({
            title: 'Success',
            text: `Room status will be changed from ${moment(statusData.startDate).format('MMM D, YYYY h:mm A')} to ${moment(statusData.endDate).format('MMM D, YYYY h:mm A')}`
          });
          ...
          const errorMessage = error.response?.data || 'Failed to update room status';
          swtoast.error({
            title: 'Error',
            text: errorMessage
          });
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            <ChangeRoomStatusForm
                ...
                setIsOpen={setIsStatusFormOpen}
                onSubmit={handleStatusSubmit}
                onCancel={handleCancel}
                isResetButtonDisplay={true}
            />
        </div>
    );
};
