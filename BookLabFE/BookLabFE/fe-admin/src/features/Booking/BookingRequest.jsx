...
const BookingRequests = () => {
  ...
  const generateFilterOptions = () => {
    ...
    const options = {
      description: [...new Set(orderDetail.map(item => item.description).filter(Boolean))],
      lectureName: [...new Set(orderDetail.map(item => item.lectureName).filter(Boolean))],
      buildingName: [...new Set(orderDetail.map(item => item.buildingName).filter(Boolean))],
      roomNumber: [...new Set(orderDetail.map(item => item.roomNumber).filter(Boolean))],
      typeSlot: [...new Set(orderDetail.map(item => item.typeSlot).filter(Boolean))],
      dates: [...new Set(orderDetail.map(item => item.date ? new Date(item.date).toLocaleDateString('en-GB') : null).filter(Boolean))],
      timeSlots: [...new Set(orderDetail.map(item => {
  ...
  const fetchBookings = async (page = 1, pageSize = 10) => {
    ...
    try {
      ...
      // Make sure we're correctly extracting the total count from the API
      // This is critical for pagination to work correctly
      const totalItems = response.data.totalCount || response.data.total || 30; // Force 30 for testing
