...
const SearchStudent = ({ setGroups, setTeam, groupName }) => {
    ...
    console.log("dm student rach : ", students)
        ...
        <div>
            <div>
                ...
                <div className="relative border-t-[2px] border-[#EEEFF1]">
                    ...
                    <input ref={inputRef} value={inputValue} onChange={(e) => { setInputValue(e.target.value) }} type="search" id="default-search" class="block w-full p-4 ps-10 text-sm text-gray-900 rounded-lg bg-gray-50 border  focus:outline-none focus:ring-2 focus:ring-[#5259C8]"
                        placeholder="Search student" required />
                    <div id="hi" className=" top-full left-0 w-full z-20 ">
                        {students != null && students.length > 0 ? (
                            <div className="rounded-[10px] border-[2px] border-[#5259C8] mt-2 backdrop-blur-md shadow-md z-30">

                                {students.map((student, index) => {
                                    return (
                                        <div id="cc23m" tabIndex={-1} key={index} onClick={() => {
